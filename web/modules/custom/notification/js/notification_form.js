...
Drupal.behaviors.notification_form = {
  attach: function(context, settings) {
    ...
    jQuery('.webform-submission-form .form-submit').click(function( event ) {
      var scrollTo = null;
    ...
    jQuery('#find_address').click(function() {
      if (!jQuery('#edit-find-address').val()) {
        alert("Enter your postcode");
        jQuery('#edit-find-address').focus();
        return false;
      }
    ...
    jQuery('#check_employer').click(function() {
      if (!jQuery('#edit-account-name').val()) {
        alert("Enter the full name of your employer");
        jQuery('#edit-account-name').focus();
        return false;
      }
      jQuery('#edit-account-address-address').val('');
      jQuery('#edit-account-address-address-2').val('');
      jQuery('#edit-account-address-city').val('');
      jQuery('#edit-account-address-postal-code').val('');
      jQuery('#employer_message').html('');
      jQuery.ajax({
        url: '/company-house/' + jQuery('#edit-account-name').val(),
        type: "GET",
        dataType: "json",
        cache: false,
        timeout: 60000,
        ...
        success: function(data){
          if (!data.length) {
            jQuery('#employer_message').html('<i>' + jQuery('#edit-account-name').val() + '</i> was not found in Companies House. If you think this is wrong, please check the spelling of your employer.');
          }
          if (data.length == 1) {
            jQuery('#edit-account-name').val(data[0].title);
            ...
            jQuery('#edit-account-address-address').val(l1);
            checkValidate(jQuery('#edit-account-address-address'));
            if (address.address_line_2) {
              jQuery('#edit-account-address-address-2').val(address.address_line_2);
            }
            if (address.locality) {
              jQuery('#edit-account-address-city').val(address.locality);
              checkValidate(jQuery('#edit-account-address-city'));
            }
            if (address.postal_code) {
              jQuery('#edit-account-address-postal-code').val(address.postal_code);
              checkValidate(jQuery('#edit-account-address-postal-code'));
            }
          }else{
            companies = data;
            var html = '<h3>More than one company found for <i>\'' + jQuery('#edit-account-name').val() + '</i>\'. Please select your employer from the list below:</h3>';
...
function populateCompany(i) {
  data = companies[i];
  jQuery('#edit-account-name').val(data.title);
