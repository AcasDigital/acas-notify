...
echo $cmd_output
cmd_output=$(/usr/bin/ssh -i /home/ubuntu/Acas-tell.pem ubuntu@34.246.85.82 'cd /var/www/html; git pull origin master' 2>&1)
if [[ $cmd_output = *"index.lock': File exists"* ]]; then
	cmd_output=$(/usr/bin/ssh -i /home/ubuntu/Acas-tell.pem ubuntu@34.246.85.82 'rm -f /var/www/html/.git/index.lock' 2>&1)
	cmd_output=$(/usr/bin/ssh -i /home/ubuntu/Acas-tell.pem ubuntu@34.246.85.82 'cd /var/www/html; git pull origin master' 2>&1)
fi
